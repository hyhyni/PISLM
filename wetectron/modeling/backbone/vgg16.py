...
from .eca_module import eca_layer, eca_layer_dilate
...
def make_layers(cfg, dim_in=3, batch_norm=False):
    ...
    for v in cfg:
        ...
        # following OICR paper, make conv5_x layers to have dilation=2
            ...
            in_channels = v
#####################################################################add by hy
    # original_eca = eca_layer(in_channels, k_size=3)
    # dilated_eca = eca_layer_dilate(in_channels, k_size=3)
    # layers.append(eca_layer(in_channels, k_size=3))

    # layers = layers[:-1]

    # layers.append(eca_layer(in_channels, k_size=3))
    # layers.append(eca_layer_dilate(in_channels, k_size=3))
#####################################################################
# remove the last relu
    return nn.Sequential(*layers[:-1])   # original
