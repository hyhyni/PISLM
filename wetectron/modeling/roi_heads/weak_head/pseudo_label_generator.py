...
from wetectron.utils import global_demo as gl
...
class mist_layer(object):
    ...
    def __call__(self, proposals, source_score, labels, device, return_targets=False):
        ...
        if num_gt_cls != 0 and num_rois != 0:
            ...
            # with open('/proposal/dual_projection_score.pkl', 'rb') as f:
            #     proposals1 = pickle.load(f, encoding='latin1')
            #     proposals_score = proposals1['dual_projection_score']
            # image_idx = gl.get_value('image_name')[0]
            # now_iter = int(gl.get_value('now_iter')) + 1
            # max_iter = cfg.SOLVER.MAX_ITER
            gt_cls_inds = labels[1:].nonzero(as_tuple=False)[:, 0]
            # lam = (log(now_iter+100)-log(100))/(log(max_iter+100)-log(100))
            # proposal_pro_score = torch.Tensor(proposals_score[image_idx][:, gt_cls_inds]).cuda()
            # sorted_scores, max_inds = (lam * cls_prob[:, gt_cls_inds] + (1-lam) * proposal_pro_score).sort(dim=0, descending=True)
            ...
            if gt_boxes.shape[0] != 0:
                ...
                lt_iou = torch.where(max_overlaps < 0.5)
                max_overlaps[lt_iou] = 1 - max_overlaps[lt_iou]
                iou_with_pseudo_GT = max_overlaps
                if return_targets:
                    matched_targets = gt_boxes[gt_assignment]
                    regression_targets = self.box_coder.encode(
                        matched_targets.bbox, proposals.bbox
                    )
                    return pseudo_labels, loss_weights, regression_targets, iou_with_pseudo_GT
